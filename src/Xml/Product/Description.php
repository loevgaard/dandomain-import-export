...
class Description extends Element
{
    ...
    public function getXml()
    {
        ...
        if ($this->descriptionShort) {
            $xml .= '<DESC_SHORT><![CDATA[' . $this->descriptionShort . ']]></DESC_SHORT>';
        }
        if ($this->descriptionLong) {
            $xml .= '<DESC_LONG><![CDATA[' . $this->descriptionLong . ']]></DESC_LONG>';
        }
        if ($this->descriptionLong2) {
            $xml .= '<DESC_LONG_2><![CDATA[' . $this->descriptionLong2 . ']]></DESC_LONG_2>';
        }
        if ($this->searchword) {
            $xml .= '<PROD_SEARCHWORD><![CDATA[' . $this->searchword . ']]></PROD_SEARCHWORD>';
        }
        if ($this->metaDescription) {
            $xml .= '<META_DESCRIPTION><![CDATA[' . $this->metaDescription . ']]></META_DESCRIPTION>';
        }
        if ($this->title) {
